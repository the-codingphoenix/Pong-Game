...
// Define canvas dimensions and positioning
const width = 500;
const height = 700;
...
// Adjust speed settings based on device
if (isMobile.matches) {
    speedY = -2;
    speedX = speedY;
    computerSpeed = 4;
...
function ballReset() {
    ballX = width / 2;
    ballY = height / 2;
    speedY = isMobile.matches ? -2 : -1; // Adjust initial speed based on device
    speedY = -3;
    paddleContact = false;
}
...
function computerAI() {
    if (playerMoved) {
        ...
        const deltaX = targetX - paddleTopX;
        paddleTopX += deltaX * 0.1; // Adjust the factor for smoothness (0.1 for smooth movement)
